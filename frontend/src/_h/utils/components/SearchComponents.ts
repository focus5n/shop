import {
  ...
  import {MenuComponent, defaultMenuOptions} from './MenuComponents'
  ...
  class SearchComponents {
    ...
    private handlers(): void {
      ...
      if (this.menuObject) {
        ...
        if (this.toggleElement) {
          ...
          this.menuObject.on('h.menu.dropdown.show', function () {
            // @ts-ignore
          ...
          this.menuObject.on('h.menu.dropdown.hide', function () {
            // @ts-ignore
            if (isVisibleElement(context.toggleElement)) {
              // @ts-ignore
        ...
        this.menuObject.on('h.menu.dropdown.shown', function () {
          // @ts-ignore
    ...
    public search = () => {
      if (!this.processing) {
        ...
        EventHandlerUtils.trigger(this.element, 'h.search.process', this)
      }
    }
    ...
    // Clear
    public clear = () => {
      if (EventHandlerUtils.trigger(this.element, 'h.search.clear') === false) {
        return
      }
      ...
      EventHandlerUtils.trigger(this.element, 'h.search.cleared')
    }
    ...
    public static getInstance = (
      ...
      const Search = DataUtils.get(el, componentName)
      if (Search) {
        return Search
      }
    ...
    public static createInstances = (
      ...
      elements.forEach((el) => {
        ...
        let Search = SearchComponents.getInstance(item)
        if (!Search) {
          Search = new SearchComponents(item, options, queries)
        }
      })
    }
    ...
    public static bootstrap = (selector: string = defaultSearchQueires.instanseQuery) => {
        SearchComponents.createInstances(selector)
    }
  ...
  export {SearchComponents, defaultSearchOptions, defaultSearchQueires}
