    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setStatus, setSubmitting }) => {
            ...
            try {
                ...
                // @ts-ignore
                const fileName = values.img[0].name


                // @ts-ignore
                const { data: product } = await productUpload(
                    values.name,
                    values.description,
                    values.price,
                    values.stock,
                    ...
                    <div className="col-lg-7">
                        ...
                        <div className="position-relative image-input image-input-empty border border-gray-300 img-fluid w-100 w-670px h-670px" data-h-image-input="true">
                            ...
                            <label className="btn btn-icon btn-circle btn-color-muted btn-active-color-primary w-25px h-25px bg-body shadow"
                                ...
                                <input id='productImage'
                                    type="file"
                                    onChange={(e) => formik.setFieldValue('img', e.currentTarget.files)}
                                    accept=".png, .jpg, .jpeg" />
                        ...
                        {formik.errors.img && (
                            <div className='fv-plugins-message-container'>
                                <span role='alert' className='text-muted ps-2'>이미지 파일을 업로드 해주세요</span>
                            </div>
                        )}
                    </div>
                            ...
                            <div>
                                <label className="position-relative me-6">
                                    <input type="radio" defaultChecked
                                        name='status'
                                        onChange={(e) => formik.setFieldValue('status', 'SALE')}
                                ...
                                <label className="position-relative me-6">
                                    <input type="radio"
                                        name='status'
                                        onChange={(e) => formik.setFieldValue('status', 'SOLDOUT')}
                                        className="position-absolute start-100 bottom-100 hidden" />
